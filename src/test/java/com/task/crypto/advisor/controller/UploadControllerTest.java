...
import com.task.crypto.advisor.service.impl.CryptoDataServiceImpl;
...
import static com.task.crypto.advisor.TestUtils.*;
...
class UploadControllerTest {
    ...
    void correctSaveRequest(MockMultipartFile multipartFile, UploadResponse expected, String crypto, int newExpectedEntriesSize) throws Exception {
        String json = performRequestWithToken(mockMvc,
                multipart(HttpMethod.POST, "/upload/upload-csv-data").file(multipartFile),
                getAdminToken(mockMvc))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        UploadResponse uploadResponse = objectMapper.readValue(json, UploadResponse.class);
        assertEquals(expected, uploadResponse);
